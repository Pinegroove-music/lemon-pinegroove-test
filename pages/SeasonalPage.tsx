...
import { Calendar, ArrowLeft, Tag, Smile, Music, Clapperboard, Disc3, Waves, Globe, Sparkles, Gift, ChevronDown, Layers, Play, Pause, Download, Loader2, Music2, Scissors } from 'lucide-react';
...
export const SeasonalPage: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedSeason) {
        fetchTracksForSeason(selectedSeason);
        setTimeout(() => {
  ...
  const fetchTracksForSeason = async (seasonName: string) => {
    setLoadingTracks(true);
    const { data, error } = await supabase.from('squeeze_tracks').select('*');
    if (!error && data) {
        const filtered = (data as MusicTrack[]).filter(track => {
            const trackSeasons = Array.isArray(track.season) ? track.season : typeof track.season === 'string' ? [track.season] : [];
            return trackSeasons.some(s => s.toLowerCase() === seasonName.toLowerCase());
        });
        setTracks(filtered);
    }
    ...
    <div className="container mx-auto px-4 pt-6 pb-32">
        ...
        <div className="mb-10 text-center">
            ...
            <p className="text-lg md:text-xl opacity-70 max-w-2xl mx-auto font-medium text-center">
                Find the perfect soundtrack for every time of the year. Hover over a category and select a theme.
            </p>
        </div>
                ...
                <div className={`group rounded-2xl overflow-hidden shadow-md transition-all duration-300 relative ${isDarkMode ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-gray-100'} hover:shadow-2xl hover:ring-2 hover:ring-sky-500/30 hover:-translate-y-1`}>
                    <div className="w-full h-20 flex items-center justify-between px-6 text-white transition-all bg-gradient-to-br from-slate-500 to-zinc-600">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-sm font-black text-left leading-tight drop-shadow-sm uppercase tracking-widest">OTHER EVENTS</h2>
                        </div>
                        <ChevronDown size={18} className="text-white/60 transition-transform group-hover:rotate-180" />
                    </div>
                    <div className="max-h-0 opacity-0 group-hover:max-h-[1000px] group-hover:opacity-100 transition-all duration-500 ease-in-out overflow-hidden">
                        <div className="p-5">
                            <div className="flex flex-wrap gap-1.5">
                                {uncategorizedSeasons.map((tag) => (
                                    <button
                                        key={tag}
                                        onClick={() => setSelectedSeason(tag)}
                                        className={`px-2.5 py-1.5 rounded-lg text-[11px] font-bold border transition-all duration-200 capitalize ${selectedSeason === tag ? 'bg-sky-500 border-sky-400 text-white' : isDarkMode ? 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:text-white hover:bg-sky-600 hover:border-sky-500' : 'bg-gray-50 border-zinc-200 text-zinc-600 hover:text-white hover:bg-sky-500 hover:border-sky-500'}`}
                                    >
                                        {tag}
                                    </button>
                                ))}
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-2xl md:text-3xl font-black tracking-tight uppercase">
                                    {selectedSeason} Tracks
                                </h2>
                                <p className="text-sm opacity-50 font-medium">Found {tracks.length} matching recordings</p>
                            </div>
                        </div>
                        
                        <button onClick={() => setSelectedSeason(null)} className="text-xs font-bold opacity-40 hover:opacity-100 flex items-center gap-1 uppercase tracking-widest">
                    ...
                    {loadingTracks ? (
                        <div className="py-20 flex flex-col items-center gap-4 opacity-50">
                            ...
                            <p className="font-bold">Gathering seasonal music...</p>
                        </div>
                    ) : tracks.length === 0 ? (
